...
# we can raise an exception here if we import subscription class and then check for valid
# subscription obj and to raise an error we have to add to class user new attribute called subscriptions
    def subscribe(self, subscription):
            if isinstance(subscription, Subscription):
                self.subscriptions.append(subscription)
            else:
                raise ValueError("Invalid subscription obj. Please provide a valid one.")